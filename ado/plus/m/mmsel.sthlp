...
where depvar (in the mmsel) is missing or equal to 0. {opt Use of 32bit Stata REQUIRED for this option}.{p_end}
...
{phang2}{cmd:. mmsel y x1 x2 x3 x4, group(female) filename(foo) reps(200) pooled}{p_end}

{title:Example with sample selection correction}

{phang2}{cmd:. mmsel y x1 x2 x3 x4, group(female) grponlysel single(z1 z2 z3) filename(foo) pooled adjust(Ps1) reduced(x1 x3) cons(x2=1&x4<56|x2=0&x4<24)}{p_end}
